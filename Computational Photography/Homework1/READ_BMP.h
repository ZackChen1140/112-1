...
bool BMP_Read(const char* filename, BYTE** pixels, int& width, int& height)
{
	...
	// Convert the const char* filename to LPCWSTR
	int wfilenameSize = MultiByteToWideChar(CP_ACP, 0, filename, -1, NULL, 0);
	if (wfilenameSize == 0) {
		printf("Error converting filename to wide string.\n");
		return false;
	}

	WCHAR* wfilename = new WCHAR[wfilenameSize];
	MultiByteToWideChar(CP_ACP, 0, filename, -1, wfilename, wfilenameSize);

	//HBITMAP bmp_handle = (HBITMAP)LoadImage(NULL, filename, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION | LR_DEFAULTSIZE);
	HBITMAP bmp_handle = (HBITMAP)LoadImage(NULL, wfilename, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION | LR_DEFAULTSIZE);
